...
func (fs *ScanDirTestSuite) SetupTest() {
	...
	err := f.Mkdir("test", 0644)
	if err != nil {
		fs.Errorf(err, "Created dir error: %v")
	}
	err = f.Mkdir("test/test_data", 0644)
	if err != nil {
		fs.Errorf(err, "Created dir error: %v")
	}
	if file, err = f.Create("test/file1.txt"); err != nil {
		fs.Errorf(err, "Created file error: %v")
	} else if _, err = file.Write([]byte("test file1")); err != nil {
		fs.Errorf(err, "Writed to file error: %v")
	}

	err = f.Mkdir("test/test_data/testDir", 0644)
	if err != nil {
		fs.Errorf(err, "Created dir error: %v")
	}
	if file, err = f.Create("test/test_data/file2.txt"); err != nil {
		fs.Errorf(err, "Created file error: %v")
	} else if _, err = file.Write([]byte("test file2")); err != nil {
		fs.Errorf(err, "Writed to file error: %v")
	}

	err = f.Mkdir("test/test_data/testDir/dir1", 0644)
	if err != nil {
		fs.Errorf(err, "Created dir error: %v")
	}
	err = f.Mkdir("test/test_data/testDir/dir2", 0644)
	if err != nil {
		fs.Errorf(err, "Created dir error: %v")
	}
	...
	err = f.Mkdir("test/test_data/testDir/dir1/dir3", 0644)
	if err != nil {
		fs.Errorf(err, "Created dir error: %v")
	}
